...
class PublishForm extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(data.baseImage === 'tomcat:8.5.13-jdk8-apm-ssl-alpine'){
      portMappings.push({
        "containerPort": 8443,
        "hostPort": 0,
        "servicePort": null,
...
//配置cmd，cmd=“”时置为null
    ...
    let { cpus, minCpus, mem, minMem} = this.state;
    if (cpus < 0.01) {
      cpus = 0.01;
    } else {
      cpus = Number(Number(this.state.cpus).toFixed(2));
    }
    if(minCpus > cpus || isNaN(minCpus)){
      minCpus = cpus;
    }else{
      minCpus = Number(Number(minCpus).toFixed(2));
    }
    if(minMem > mem || isNaN(minMem)){
      minMem = mem;
    }
...
//如果是dubbo类型，做一些change
    ...
    if(window.YHTTool.isUseUkey){
      window.YHTTool.isUseUkey((res) => {
        console.log('是否需要u盾验证',res);
        if(res){
          window.YHTTool.checkCA((res) => {
            console.log('验证u盾',res);
            if(res){
              pub();
            }else{
              err('u盾验证未通过');
    ...
    const pub = () => {
      if (this.state.isK8s) {
        if (this.state.minMem.toString().indexOf('.') > -1) {
          this.publishFlag = false;
          return warn('内存必须是整数。');
        }
        if (Number(this.state.minMem) > this.state.mem) {
          this.publishFlag = false;
          return warn('内存的最小值应该小于最大值');
        }
        if (Number(this.state.minCpus) > this.state.cpus) {
          this.publishFlag = false;
          return warn('CPU的最小值应该小于最大值');
        }
...
//先校验mesos版本，设置请求
  ...
  onCheckboxChange = (id, type) => (e) => {
    ...
    let {resChecked, resPool, portMappings} = this.state;
        ...
        <Col lg={8} lgOffset={1} md={10} mdOffset={1} sm={12} smOffset={0}>
              ...
              </Col>
              {
                this.state.isK8s ? (
                  <Col xs={9} className="padding-0">
                    ...
                    <Col sm={12}>
                      <Label>{`内存(最大：${this.state.memLeft})`}</Label>
                    </Col>
                    <Col sm={2}>
                      <Label>最小值</Label>
                    </Col>
                    <Col sm={4} xs={6}>
                      <InputGroup>
                        <FormControl
                          style={{imeMode: 'Disabled'}}
                          onKeyDown={ onlyNumber }
                          onChange={this.handleInputChange('minMem')}
                          // value={ this.state.minMem < this.state.mem || this.state.isPrivateRes ? this.state.minMem : this.state.mem}
                    ...
                    <Col sm={12}>
                      <Label>{`CPU(最大：${this.state.cpuLeft})`}</Label>
                    </Col>
                    <Col sm={2}>
                      <Label>最小值</Label>
                    </Col>
                    <Col sm={4} xs={6}>
                      <FormControl
                        style={{imeMode: 'Disabled'}}
                        onKeyDown={ onlyNumber }
                        onChange={this.handleInputChange('minCpus')}
                        // value={ this.state.minCpus < this.state.cpus || this.state.isPrivateRes ? this.state.minCpus : this.state.cpus}
                        value={this.state.minCpus}
                      />
                    </Col>
                    <Col sm={2}>
                      <Label>最大值</Label>
          ...
          <Col xs={9} xsOffset={3}>
            <div className="btn-group">
              <Button colors='danger' shape="squared" onClick={this.handleSubmit.bind(this)}
