...
import { getAppList as GetPublishList} from '../../../serves/panelObsv';
...
import MAPPER from '../const/timeMapper';
let { notOther } = MAPPER
...
class Main extends Component {
    constructor(props) {
        ...
        this.state = {
            timerOther: !notOther(MAPPER[getQuery().timeId]),
            empty: 'none',
            timer: MAPPER.name[MAPPER[getQuery().timeId]] || '其它时间段',
            open: false,
            pv: getOption('pv'),
            uv: getOption('uv'),
            ...
            more: 'none',
            appList: [],
            time: MAPPER[getQuery().timeId] || '15min',
            appId: '',
            appIdInNum: '',
            appName: '',
            pvNo: 'none',
    ...
    getAppList() {
        let self = this;
        return GetPublishList().then(function (res) {
            let appList =res;
            if (appList && appList.length) {
                let appId = getQuery().appId || appList[0].app_id;
                let appIdInNum = getQuery().appIdInNum || appList[0].number_app_id;
                let appName = getName(appList, getQuery().appId) || appList[0].app_name;
                ReactDOM.findDOMNode(self.refs.operation).className = "u-row operation-m";
                self.setState({
                    appId: appId,
                    appIdInNum:appIdInNum,
                    appList: appList,
                    appName: appName,
                    operation: 'block'
                });
            ...
            <div>
                ...
                <Row className="operation-m opacity" ref="operation" style={{ 'display': this.state.operation }}>
                    ...
                    <div className="top clearfix">
                            ...
                            >
                                {
                                    this.state.appList.map(function (item) {
                                        return <Option key={item.id} value={item.app_id} label={item.app_name}>{item.app_name}</Option>
                                    })
                                }
                            </Select>
                        ...
                        <div className="time">
                            ...
                            <ButtonGroup >
                                ...
                                <Button shape='squared' className={classnames({ 'u-button': true, 'active': this.state.time == '1w' })} bordered data-time="1w" onClick={this.timeClick}>一周</Button>
                            </ButtonGroup>
                    ...
                    <div className="switches">
                        ...
                        <Link
                            to={`/userAct?appIdInNum=${this.state.appIdInNum}&appId=${this.state.appId}&timeId=${MAPPER[this.state.time]}`}
                            className="u-button switch-btn"
                            activeClassName="switch-btn__active">
                            用户行为分析
                        </Link>
                        <Link
                            to={`/browser?appIdInNum=${this.state.appIdInNum}&appId=${this.state.appId}&timeId=${MAPPER[this.state.time]}`}
                            className="u-button switch-btn"
                            activeClassName="switch-btn__active"
                        >
                            浏览器性能分析</Link>
                        <Link
                            to={`/service?appIdInNum=${this.state.appIdInNum}&appId=${this.state.appId}&timeId=${MAPPER[this.state.time]}`}
                            className="u-button switch-btn"
                            activeClassName="switch-btn__active"
                        >
                            应用业务分析</Link>
