...
import { Select, Message, Button, Switch,Row } from 'tinper-bee';
import { Link, withRouter } from 'react-router';

import VerifyInput from 'components/verifyInput/index';
import PageLoad from '../../components/pageLoad-middleware';
import ConfPanel from '../../components/confPanel-middleware';

import withStyle from '../../withStyle.hoc';
import Header from '../../components/header-middleware';
import Password from '../../components/password-middleware';
...
const STORAGE_TYPE = 'zk';

class CreateMysqlPage extends Component {
    static propTypes = {};
    static defaultProps = {};
    static contextTypes = {};
    ...
    handleCreateSrv = (evt) => {
        ...
        let { databaseName, srvDesc, srvConf, srvVersion = '2.8.19' } = this.state;
    ...
    handlePassWDAlgSwitch = (flag) => {
        if (flag) {
            ...
            this.errStatus.srvPassword = true;
            this.errStatus.srvPasswordBack = true;
        } else {
            this.setState({
                autoPW: flag,
                autoPassword: '',
    ...
    render() {
        ...
        if (this.state.loading) {
            ...
            return (
                <Row>
                    <div style={{ height: '100%' }} className="zkWraper">
                        <Header widthGoBack="true" router={this.props.router}>
                            <span>创建我的ZooKeeper服务</span>
                        </Header>
