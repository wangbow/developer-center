...
class ListPage extends Component {
  ...
  managerAuth = (rec) => {
    return (e) => {
      e.stopPropagation();
      verifyAuth('zk', rec, () => {
        this.context.router.push(`/auth/${rec.insName}?id=${rec.pkMiddlewareZk}&userId=${rec.userId}&providerId=${rec.providerId}&backUrl=md-service&busiCode=middleware_zk`);
      })
    }
  }
  ...
  renewalSelectedInstance = () => {
    renew(splitParam({id: this.__modalPayLoad[0].pkMiddlewareZk}), STORAGE_TYPE)
      .then(() => {
        this.setState({ showModal: false });
        this.handleRefresh();
      })
