...
import {Table, Button, InputGroup, FormControl, Icon, Popconfirm, Select, Pagination, CheckBox, Message} from 'tinper-bee';
...
class ConfigCenter extends Component {
    ...
    componentDidMount() {
        ...
        if(confCenterId != 0){
            ...
            //获取环境列表
            GetConfigEnvFromCenter(function (res) {
    ...
    getVersion = (env) => {
        ...
        if(appCode !== ''){
            //获取版本列表
            GetConfigVersionFromCenter(`?appCode=${ appCode }&envId=${ env }&needDefine=false`, function (res) {
                if (res.data.success === 'true') {
                    ...
                    self.getConfigFile(env, version);
                }else{
                    self.setState({
                        error: true,
                    });
    ...
    getConfigFile = (env, version, page) => {
        ...
        if(version === ''){
            ...
            GetConfigFileFromCenter(`?appCode=${ appCode }&envId=${ env }&version=${ version }&pgSize=9&pgNo=${ page ? page : activePage }`, function (res) {
                if(res.data.success === 'false'){
                    ...
                    let totalpage = Math.floor(res.data.page.totalCount / 9) + 1;
    ...
    handleDelete = (record, index) => {
        let { data } = this.state;
        return () => {
            deleteConfigFile(record.configId, (res) => {
                if(res.status === 200){
                    data.splice(index, 1);
                    this.setState({
                        data
