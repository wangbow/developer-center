...
import {Icon, PanelGroup, Panel, Table, Button, Popover, Modal, Popconfirm, Tooltip} from 'tinper-bee';
import TimeLineModal from '../timeLine-modal';
import {DownloadWar} from 'serves/appTile';
import {batchDelete, getTimeLine} from 'serves/CI';
import imgempty from 'assets/img/taskEmpty.png';
import {lintAppListData, dataPart} from 'components/util';
...
class VersionList extends Component {
  constructor(props) {
    ...
    this.columns = [
      ...
      },
      {
        ...
        key: 'version',
        render: (text) => {
          return (<p style={{width: 250, wordBreak: 'break-all'}}>{text}</p>)
        }
      },
      {
        title: '上传时间',
   ...
   */
  handleDelete = () => {
    const {refreshList, list} = this.props;
  ...
  render() {
    const {list, onShowUpload, refreshList} = this.props;
