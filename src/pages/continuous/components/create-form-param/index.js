...
import { verifyImage, getUploadWarInfo, getBaseDocker } from 'serves/CI';
      ...
      <div className="create-form-param">
        ...
        <div>
          <FormGroup>
            ...
            <ul className="select">
              {
                this.state.baseImage.map((item) => {
                  return (
                    <li
                      key={item.imageName}
                      className={ classnames({"active": this.state.selectedInv === item.imageName})}
                      onClick={ this.selectBaseImage(item.imageName)}>
