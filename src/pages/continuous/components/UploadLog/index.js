...
import {Button, Col, InputGroup, FormControl, Message, Popover} from 'tinper-bee';
import OverlayTrigger from 'bee-overlay/build/OverlayTrigger';

import {getUploadLog, getUploadConsole} from 'serves/CI';
...
class UploadLog extends Component {
  ...
  componentDidMount() {
    ...
    let {isDone, data, error_app} = this.props;
  ...
  componentWillReceiveProps(nextProps) {
    let {isDone, data, error_app} = nextProps;
    let flag = true;
    let clear = false;
  ...
  getUploadConsole = (appUploadId, buildVersion, empty, clear) => {
    this.setState({showPauseFlag: true});
    console.log('log----', this.timer);
    //if (!this.timer) {
  ...
  getUploadConsoleFromDb = (appUploadId, buildVersion) => {
    this.setState({showPauseFlag: false});
   ...
   */
  searchKey = () => {
    let {formatConsoleData, cleanData} = this.state;
    ...
    if (node != "" && keyWord != "") {
      ...
      this.setState({formatConsoleData});
    }
    let patternFont = new RegExp('<font', "gi");
    this.setState({keyStep: -1});

    if (keyWord == "") {
      this.setState({formatConsoleData: cleanData});
      this.setState({consoleData: cleanData});
    }
  ...
  nextKey = (num) => {
    ...
    return () => {
      ...
      self.setState({keyStep: currentStep});
  ...
  pauseConsole = () => {
    if (!this.state.togglePauseFlag) {
      ...
      this.getUploadProgress(this.state.appId, this.splitLatestVersion(this.state.currentBuildVersionForConsole));
    }
          ...
          <div className="console-edit">
            {
              ...
              (
                <Button className="edit-icon" size="sm">
                  {
