...
import {lintAppListData, checkEmpty} from 'components/util';
...
class UploadDetail extends Component {
  ...
  getRes = () => {
    GetResPoolInfo( (res) => {
      let resPoolInfo = lintAppListData(res);
      this.setState({
        resPoolInfo
      });
  ...
  getBranchListEncryp = (data) => {
    ...
    if(data.ssh_flag) {
      ...
      getBranchListEncryp(`?gitUrl=${encodeURIComponent(url)}&gitUserName=${encodeURIComponent(name)}&gitPassWord=${encodeURIComponent(password)}`)
        .then((res) => {
          let data = res.data;
          if (data.error_code) {
            return err(data.error_message)
   ...
   * 一键部署
   */
  handleQuickPublish = (envList, checkRes = []) => {
    let {publishInfo, totalVersionInfo, data} = this.state;
    if (!publishInfo || publishInfo.length === 0)
      return warn('没有部署过的应用无法一键部署，请先部署应用。');

    let host = `http://${window.location.host}/confcenter`;
    let paramData = [], prarmDataHasConf = [];
    let getFileFunction = [];
    ...
    publishInfo.forEach((item) => {
      if(envList.indexOf(item.app_type) > -1){
        let paramObj = {
          ...
          confProperties: null,
          res_pool_ids: checkRes
        };

        if(item.cf_disconf_env != null && item.cf_disconf_version != null){
          getFileFunction.push(this.getFile(item.cf_disconf_env, item.cf_disconf_version));
          paramObj.confProperties = {
            "confServerHost": host,
            "confVersion": item.cf_disconf_version,
            "confApp": data.appCode,
            "confEnv": item.cf_disconf_env,
            "confUserDefineDownloadDir": data.filePath,
            "confUserDefineConfigFile": ''
          }
          prarmDataHasConf.push(paramObj);
        }else{
    ...
    if(getFileFunction.length !== 0){
      Promise.all(getFileFunction).then(function () {
        prarmDataHasConf.forEach((item, index) => {
          item.confProperties.confUserDefineConfigFile = arguments[0][index]
        });
        quickPublish(paramData.concat(prarmDataHasConf))
          .then((res) => {
            let data = res.data;
            if (data.error_code)
              return err(data.error_message);
  ...
  getFile = (env, version) => {
    let {data} = this.state;
      let file = [];
      return GetConfigFileFromCenterByCode(`?appCode=${ data.appCode }&envId=${ env }&version=${ version }`)
        .then((res) => {
          ...
          <Tabs
            ...
            <TabPane tab="一键部署" key="4">
              <QuickPublish
               ...
               appCode={ data.appCode }
               resPool={ this.state.resPool }
               resPoolInfo={ this.state.resPoolInfo }
